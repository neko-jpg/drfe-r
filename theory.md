...
定理 1 (DRFE-R の到達保証):  
グラフ $G$ が連結であり、かつパケットのTTL内で切断されない限り、GPルーティングを用いた DRFE-R は、送信元からホームノードへ、そしてホームノードから宛先ノードへのパケット到達を保証する。  
これにより、「座標の振動」や「埋め込みの歪み」が存在しても、パケットがネットワーク内で消失することを理論上防ぐことができる。

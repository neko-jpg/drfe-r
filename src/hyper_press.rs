...
impl HyperPress {
    ...
    /// - The previous LPA scheme often destroys locality on real AS graphs, creating many local minima.
    ///
    /// Method (O(N+E)):
    ...
    pub fn compute_potential(&self, target: &NodeId) -> HashMap<NodeId, f64> {
        ...
        for _ in 0..self.potential_iterations {
            for (node, neighbors) in &self.adjacency {
                ...
                // φ_new(u) = (1/deg(u)) · Σ_{v ∈ N(u)} φ(v)
                let sum: f64 = neighbors.iter()
                    .filter_map(|n| phi.get(n))
                    .sum();
                let new_val = sum / (neighbors.len() as f64);
                phi.insert(node.clone(), new_val);
            }
        }
